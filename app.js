...
async function bxh() {
  ...
  const getBxhApi = await fetch ('https://mp3.zing.vn/xhr/media/get-source?type=album&key=ZnxHtLzuWQcdCZByHtDHkmtLCNvmpkVWz')
...
let minsec = `${min}:${sec}`
  ...
  if (Math.floor(music.currentTime) == Math.floor(music.duration)) {
    ...
    let songBxhSource = songbxh.source[128]
    const linkSrcMusic = `https://vnno-vn-5-tf-` + songBxhSource.slice(2, songBxhSource.length)
// không dùng cái đuôi /360 hay 128 vì nó trải qua 2 link để đến audio (github chỉ lấy link đầu, mà link đầu notwork -> lỗi)
...
//    }
  ...
  // "replace (/ /g, "")" remove all whitespace
    ...
    let oblistSource = oblist.source[128]
    const linkmp = `https://vnno-vn-5-tf-` + oblistSource.slice(2, oblistSource.length)
